...
/* default: non-realtime task. murray mod */
int pth_attr_init(pth_attr_t a)
{
    ...
    a->a_exetime.tv_sec = 0;
    a->a_exetime.tv_usec = 0;
    a->a_period.tv_sec = 0;
    a->a_period.tv_usec = 0;
    a->a_type = TSTASK;
...
/* murray added 3 cases: EXETIME PERIOD TYPE */
intern int pth_attr_ctrl(int cmd, pth_attr_t a, int op, va_list ap)
{
    ...
    switch (op) {
        ...
        case PTH_ATTR_EXETIME: {
            /* execute time */
            pth_time_t *src, *dst;
            if (cmd == PTH_ATTR_SET) {
                src = va_arg(ap, pth_time_t *);
                dst = (a->a_tid != NULL ? &a->a_tid->exe_time : &a->a_exetime);
            }
            else {
                src = (a->a_tid != NULL ? &a->a_tid->exe_time : &a->a_exetime);
                dst = va_arg(ap, pth_time_t *);
            }
            *dst = *src;
            break;
        }
        case PTH_ATTR_PERIOD: {
            /* execute period */
            pth_time_t *src, *dst;
            if (cmd == PTH_ATTR_SET) {
                src = va_arg(ap, pth_time_t *);
                dst = (a->a_tid != NULL ? &a->a_tid->period : &a->a_period);
            }
            else {
                src = (a->a_tid != NULL ? &a->a_tid->period : &a->a_period);
                dst = va_arg(ap, pth_time_t *);
            }
            *dst = *src;
            break;
        ...
        case PTH_ATTR_TYPE: {
            ...
            if (cmd == PTH_ATTR_SET) {
                src = &val; val = va_arg(ap, int);
                dst = (a->a_tid != NULL ? &a->a_tid->flag : &a->a_type);
            }
            else {
                src = (a->a_tid != NULL ? &a->a_tid->flag : &a->a_type);
                dst = va_arg(ap, int *);
            }
            *dst = *src;
            break;
