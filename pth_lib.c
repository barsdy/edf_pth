...
int pth_init(void)
{
    ...
    /* Murray added begin */
    /* set timer for tick schedule */
    sigset_t pth_timer_sigset;
    struct sigaction pth_timer_handler;
    struct itimerval pth_tick_time;

    sigemptyset(&pth_timer_sigset);
    sigaddset(&pth_timer_sigset, SIGALRM);

    pth_timer_handler.sa_handler = pth_tick_scheduler;
    pth_timer_handler.sa_flags = SA_RESTART;
    pth_timer_handler.sa_mask = pth_timer_sigset;

    if (sigaction(SIGALRM, &pth_timer_handler, NULL)) {
        pth_debug2("sigaction fail:[%s]\n", strerror(errno));
        return FALSE;
    }

    /* Now we just set the interval to 20ms */
    pth_tick_time.it_interval.tv_sec = 0;
    pth_tick_time.it_interval.tv_usec = PTH_INTERVAL;
    pth_tick_time.it_value.tv_sec = 0;
    pth_tick_time.it_value.tv_usec = 20000;

    if (setitimer(ITIMER_REAL, &pth_tick_time, NULL)) {
        pth_debug2("setitimer fail:[%s]\n", strerror(errno));
        return FALSE;
    }
    pth_debug1("pth_init: murray timer");
    /* Murray added end */
    ...
    pth_attr_set(t_attr, PTH_ATTR_STACK_ADDR,   NULL);
    /* murray added for real-time */
    pth_attr_set(t_attr, PTH_ATTR_TYPE, TSTASK);
...
pth_t pth_spawn(pth_attr_t attr, void *(*func)(void *), void *arg)
{
    ...
    if (attr != PTH_ATTR_DEFAULT) {
        if (attr->a_type & PTH_TASK_RT) {
            if (pth_time_t2d(&attr->a_period) == 0.0) {
                pth_debug2("pth_spawn: task [%s] has an empty period!\n", attr->a_name);
                return pth_error((pth_t)NULL, EINVAL);
            }
            ...
            pth_debug4("pth_spawn: accecp the rt task:[%s]. C=[%.6f], P=[%.6f].", \
                    attr->a_name, pth_time_t2d(&attr->a_exetime), pth_time_t2d(&attr->a_period));
            pth_debug4("pth_spawn: scheduler overload:[%f],keyload:[%f], task load:[%f]", \
                    pth_get_edfload(), pth_get_keyload(), tmp_load);

            pth_time_set(&t->exe_time, &attr->a_exetime);
            pth_time_set(&t->period, &attr->a_period);
            pth_time_set(&t->remain, &t->exe_time);
            pth_time_set(&t->deadline, PTH_TIME_NOW);
            pth_time_add(&t->deadline, &t->period);
            t->task_load = tmp_load;
        }
...
void pth_exit(void *value)
{
    ...
    if (pth_current->flag & PTH_TASK_RT) 
        pth_sub_load(pth_current->task_load, pth_current->flag & PTH_TASK_HARD);
...
int pth_mod_main_attr(pth_attr_t attr)
{
    float tmp_load;
    ...
    if (attr->a_type & RTTASK) {
        pth_time_set(&pth_main->exe_time, &attr->a_exetime);
        pth_time_set(&pth_main->period, &attr->a_period);
        pth_time_set(&pth_main->deadline, PTH_TIME_NOW);
        pth_time_add(&pth_main->deadline, &attr->a_period);
        pth_time_set(&pth_main->remain, &attr->a_exetime);
        pth_main->flag = attr->a_type;
