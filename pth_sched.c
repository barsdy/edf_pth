...
intern void pth_scheduler_block(pth_time_t until)
{
    ...
    if ((ev = pth_event(PTH_EVENT_TIME|PTH_MODE_STATIC, &ev_key, until)) == NULL) {
        pth_debug2("pth_scheduler: get time event fail:[%s]\n", strerror(errno));
        return;
    }

    ev->ev_status = PTH_STATUS_PENDING;

    pth_current->events = ev; /* come on! gvie me a fucking shit */
...
intern void *pth_scheduler(void *dummy)
{
    ...
    __sync_fetch_and_or(&pth_sched_working, 1); /* set pth_sched_working to 1 */
    ...
    for (;;) {
        ...
        while ((t = pth_pqueue_tail(&pth_NQ)) != NULL) {
            ...
            */
            ...
            pth_debug2("pth_scheduler: new thread \"%s\" moved to ready queue", t->name);
        }
        ...
        /* 
         * Check the deadline is over date
         */
        if (pth_current->flag & PTH_TASK_RT) {
            c_deadline = pth_time_t2d(&pth_current->deadline) - pth_time_t2d(&snapshot);
            if (c_deadline < 0) {
                pth_debug3("pth_scheduler: the real-time task:[%s] has passed the deadline:[%f]!!!", \
                        pth_current->name, c_deadline);
                pth_time_set(&pth_current->deadline, &snapshot);
                pth_time_add(&pth_current->deadline, &pth_current->period);
                pth_time_set(&pth_current->remain, &pth_current->exe_time);
                c_deadline = pth_time_t2d(&pth_current->exe_time);
            }
            pth_debug4("pth_scheduler: thread \"%s\" selected (remain exectime=%f, deadline=%f)",
                       pth_current->name, pth_time_t2d(&pth_current->remain), c_deadline);
        }
        ...
        /* set pth_ticks */
        remain_time = pth_current->remain.tv_sec * 1000000 + pth_current->remain.tv_usec;
        pth_ticks = (pth_current->flag & PTH_TASK_RT) ? (int)(remain_time / PTH_INTERVAL) : -1;
       ...
       */
        ...
        /* 
         * murray added. Update things related to edf alg
         */
        if (pth_current->flag & PTH_TASK_RT) {
            pth_time_sub(&pth_current->remain, &running);
            c_deadline = pth_time_t2d(&pth_current->deadline) - pth_time_t2d(&snapshot);
            /* pth_time_sub made tv_usec cannot < 0 */
            if (pth_current->remain.tv_sec < 0 || c_deadline <= 0 || \
                    (pth_current->remain.tv_sec == 0 && pth_current->remain.tv_usec == 0)) {
                /* execute time up */
                if (pth_current->flag & PTH_TASK_PERIOD) {
                    /* reset remain time and deadline */
                    ...
                    /* TODO: avoid these duplicated code */
                    if (pth_current->cleanups != NULL)
                        pth_cleanup_popall(pth_current, TRUE);

                    if (pth_current->data_value != NULL)
                        pth_key_destroydata(pth_current);

                    pth_mutex_releaseall(pth_current);

                    pth_current->state = PTH_STATE_DEAD; 
                    pth_debug2("pth_scheduler: rt thread [%s] abolished!", pth_current->name);
                }
            }
            else {
         ...
         */
        /* deleted by murray: static prio is fine
 ...
 */
/* 
 * murray modified. Add a dopoll type -- "CHECK" for tick scheduler
 * and modified the func return type. 1 means events happened, 0 means no event happen.
 */
intern int pth_sched_eventmanager(pth_time_t *now, int dopoll)
{
