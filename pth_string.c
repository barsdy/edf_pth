    ...
    int flags)
{
    ...
    /* murray debug for 0.01 turn to 0.1 situation */
    int fracplace = 0;
    long cntplace = fracpart;
    while(cntplace) {
        cntplace = (long)(cntplace / 10);
        fracplace++;
    }
    int offset = max - fracplace;
/*
printf("murray debug. offset:[%d], max:[%d], fracplace:[%d]\n", offset, max, fracplace);
*/
    do {
        fconvert[fplace++] =
            (caps ? "0123456789ABCDEF"
    ...
    while (offset-- > 0) 
        fconvert[fplace++] = '0';
/*
printf("murray debug. fconvert:[%s]\n", fconvert);
*/
    if (fplace == 20)
        fplace--;
    fconvert[fplace] = 0;
